...
import { addDeliverableCommentRecord } from "@/lib/supabase/deliverables";
import { supabaseMutationResponse } from "@/lib/supabase/http";
import { deliverableCommentSchema, zodFieldErrors } from "@/lib/validation";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!hasPermission(workspace.role, "comments.write")) return apiError("FORBIDDEN", "Seu papel não permite comentar.", 403);
  ...
  const parsed = deliverableCommentSchema.safeParse(payload);
  if (!parsed.success) return apiError("VALIDATION_ERROR", "Revise o comentário.", 422, zodFieldErrors(parsed.error));
  const { id } = await context.params;
  return supabaseMutationResponse(await addDeliverableCommentRecord(workspace, id, parsed.data), 201);
}
