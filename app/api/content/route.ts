...
import { createContentItem } from "@/lib/supabase/content";
import { isSupabaseConfigured } from "@/lib/supabase/config";
import { supabaseMutationResponse } from "@/lib/supabase/http";
import { contentItemSchema, zodFieldErrors } from "@/lib/validation";
...
export async function POST(request: Request) {
  ...
  if (!hasPermission(workspace.role, "content.write")) return apiError("FORBIDDEN", "Seu papel não permite criar conteúdo.", 403);
  ...
  const parsed = contentItemSchema.safeParse(payload);
  if (!parsed.success) return apiError("VALIDATION_ERROR", "Revise os campos destacados.", 422, zodFieldErrors(parsed.error));
  return supabaseMutationResponse(await createContentItem(workspace, parsed.data), 201);
}
