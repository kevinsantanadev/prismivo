...
async function getAppStyleUrls(page: Page) {
  await page.goto("/");
...
async function renderStyledFixture(page: Page, styleUrls: string[], markup: string) {
...
test("Marco 23 keeps agenda and routines usable across the phone matrix", async ({ page }) => {
  const styleUrls = await getAppStyleUrls(page);

  for (const width of [320, 360, 375, 390, 393, 412, 430]) {
    await page.setViewportSize({ width, height: 844 });
    await renderStyledFixture(
      page,
      styleUrls,
      `
...
test("the overview compacts a remaining widget instead of leaving an empty column", async ({ page }) => {
  ...
  for (const width of [1280, 390]) {
    ...
    await renderStyledFixture(
      page,
      styleUrls,
      `<main class="app-content"><section class="dashboard-widget-layout"><div class="dashboard-widget-slot widget-pulse"><article class="dashboard-panel"><h2>Saúde da operação</h2><p>Conteúdo operacional</p></article></div></section></main>`,
    );
