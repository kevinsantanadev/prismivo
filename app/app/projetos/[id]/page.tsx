...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell active="projects" title={project.name} description="Central operacional do projeto" workspace={workspace} unreadCount={unreadCount}>
    ...
    <section className="detail-hero project-detail-hero">
      ...
      <div className="detail-progress"><strong>{project.progress}%</strong><span>{copy.projects.detail.overallProgress}</span><ProjectProgress id={project.id} progress={project.progress} locale={locale} /></div>
    </section>
    ...
    <ProjectDeliverables projectId={project.id} items={deliverables} canWrite={hasPermission(workspace.role, "deliverables.write")} canComment={hasPermission(workspace.role, "comments.write")} locale={locale} />
    <section className="dashboard-panel detail-section">
      <div className="detail-section-heading"><div><span className="panel-kicker">{copy.projects.detail.privateFiles}</span><h2>{copy.projects.detail.documents}</h2></div><FileUpload projects={[{ id: project.id, name: project.name }]} fixedProjectId={project.id} locale={locale} /></div>
