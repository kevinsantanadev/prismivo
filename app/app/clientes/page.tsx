...
import { findWorkspaceByEmail, getClientsData, getProjectsData, getUnreadNotificationCount } from "@/lib/workspace";
...
export default async function ClientsPage() {
  ...
  const [clients, projectsData, unreadCount, locale] = await Promise.all([
  ...
  const insights = new Map(deriveClientInsights(clients, projectsData).map((insight) => [insight.clientId, insight]));
  const clientsWithInsights = clients.map((client) => ({ ...client, insight: insights.get(client.id)! }));
  ...
  return (
    <AppShell active="clients" title="Clientes" description="Carteira protegida por empresa" workspace={workspace} unreadCount={unreadCount}>
      ...
      <ClientDirectory clients={clientsWithInsights} locale={locale} />
    </AppShell>
  );
}
