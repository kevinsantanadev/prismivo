    ...
    <section className="dashboard-panel" aria-labelledby="members-title">
      ...
      <div className="member-list">{members.map((member) => {
        ...
        return <article className="member-row" key={member.id}>
          <div className="member-identity"><span>{initials}</span><div><strong>{member.name}{member.userId === currentUserId ? ` (${copy.you})` : ""}</strong><small>{roleName(member.role)} · {copy.statuses[member.status as keyof typeof copy.statuses] ?? member.status}</small></div></div>
