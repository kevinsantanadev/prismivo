...
As jornadas E2E rodam em Chromium somente depois da validação estática e cobrem entrada gratuita, preferências, proteção de rotas, responsividade e disponibilidade pública. `/api/health` fornece prontidão técnica sem dados sensíveis, enquanto `/status` comunica o estado de forma humana.
