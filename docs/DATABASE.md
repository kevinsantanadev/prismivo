...
O ambiente independente usa PostgreSQL 17 no Supabase e migrações SQL versionadas. Estão implementadas quatorze tabelas públicas: `profiles`, `organizations`, `memberships`, `organization_invitations`, `clients`, `projects`, `approvals`, `tasks`, `files`, `support_tickets`, `ticket_messages`, `activities`, `notifications` e `consents`.
...
Essa fatia sustenta cadastro, isolamento por organização, colaboração, convites, perfis profissionais, avatares privados, clientes, projetos, tarefas, aprovações, arquivos, atendimentos, histórico, notificações, preferências e consentimentos.
...
Os índices do marco atual já cobrem e-mail e slug únicos, participação usuário/empresa, clientes, projetos, tarefas por status/responsável, arquivos ativos, chamados, mensagens, aprovações, notificações e atividades.
