...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell active="projects" title={project.name} description="Central operacional do projeto" workspace={workspace} unreadCount={unreadCount}>
    ...
    <section className="dashboard-panel detail-section">
      <div className="detail-section-heading"><div><span className="panel-kicker">{copy.projects.detail.privateFiles}</span><h2>{copy.projects.detail.documents}</h2></div><FileUpload projects={[{ id: project.id, name: project.name }]} fixedProjectId={project.id} /></div>
